...
export default function AdminEditPostPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/posts/${id}`)
      ...
      .then((data) => {
        if (data && !data.error) {
          setFormData({ ...data, faqs: data.faqs || [] });
        }
      })
  ...
  if (loading) {
    return <div className={`p-8 font-mono text-xs ${isDark ? "text-[#737373]" : "text-[#94a3b8]"}`}>Loading article data...</div>;
  }
  ...
  return (
    <div>
      <AdminHeader
        title="Edit Article"
        description={`Updating article: ${formData.title}`}
        actions={
          <Link
            href="/admin/blog"
